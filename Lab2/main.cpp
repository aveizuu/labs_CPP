...
const int count_of_eqs = 50;
const int count_of_students = 10;
void generateEquations(const string filename) {
...
const string name[10] = { "Andreev Yaroslav", "Shildt Daniel", "Bruchko Uliana", "Gonrov Ilya", "Kozachenko Kristina", "Mamitov Timur", "Dokukina Nadejda", "Belyaev Oleg", "Ivanov Maksim", "Maksutov Ivan" };
...
int main(void) {
	srand(time(0));
	generateEquations("equations.txt");
	vector<unique_ptr<Student>> students = generateStudents();

	Teacher teacher;

	studentsSolveEqs("equations.txt", students, teacher);
